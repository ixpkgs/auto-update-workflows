...
Reusable GitHub Actions workflows for discovering new upstream software
versions and updating packaged software. Versions can be discovered from Git
tags or published GitHub Releases, and updater workflows can consume the
result through a shared interface.

## Available workflows

| Workflow | Purpose |
| --- | --- |
| `.github/workflows/discover-upstream.yml` | Discovers unprocessed upstream versions and returns them as structured data |
| `.github/workflows/update-go-module.yml` | Updates a Nix flake package built with `buildGoModule`, verifies each build, and publishes the resulting commits and tags |
...
`.github/workflows/update-go-module.yml` updates Nix flakes that package tagged
Go versions with `buildGoModule`.
...
jobs:
  update:
    uses: ixpkgs/auto-update-workflows/.github/workflows/update-go-module.yml@COMMIT_SHA
    with:
...
To package only published GitHub Releases, configure the caller as follows:
...
The selected flake file must be tracked by Git and contain exactly one
assignment of each of these forms:
